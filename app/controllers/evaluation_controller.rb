class EvaluationController < ApplicationController
  ...
  def new
    ...
    elsif Tweet.find_by_sql(['SELECT t.* FROM tweets t LEFT OUTER JOIN votes v ON t.tweet_id = v.tweet_id AND v.user_id = :user LEFT OUTER JOIN doing_lists d ON t.tweet_id = d.tweet_id WHERE t.votes_count < 6 AND v.user_id IS NULL AND d.user_id IS NULL ORDER BY t.votes_count DESC', {user: current_user.id}]).length > 0 then
      @tweets = Tweet.find_by_sql(['SELECT t.* FROM tweets t LEFT OUTER JOIN votes v ON t.tweet_id = v.tweet_id AND v.user_id = :user LEFT OUTER JOIN doing_lists d ON t.tweet_id = d.tweet_id WHERE t.votes_count < 6 AND v.user_id IS NULL AND d.user_id IS NULL ORDER BY t.votes_count DESC', {user: current_user.id}]).first(10)
  ...
  def create
    ...
    yes_count = Vote.where(tweet_id: tweet_id, evaluation: 1).count
    no_count = Vote.where(tweet_id: tweet_id, evaluation: 0).count
    Tweet.where(:tweet_id => tweet_id)[0].update_column(:accept_count, yes_count)
    Tweet.where(:tweet_id => tweet_id)[0].update_column(:reject_count, no_count)
    Tweet.where(:tweet_id => tweet_id)[0].save
    if Tweet.where(:tweet_id => tweet_id)[0].accept_count > 2 then
      Tweet.where(:tweet_id => tweet_id)[0].update_column(:confirm, 1)
